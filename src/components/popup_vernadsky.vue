...
<script>
  ...
  export default {
    emits: ['navigateToMainK', 'navigateToMainUust', 'navigateToNotsRb'], // Объявляем событие
    setup(_, {emit}) {
      ...
      const defaultPointStyle = new Style({
        image: new Circle({
        radius: 7,
        fill: new Fill({ color: 'rgba(150,105,200,255)' }),
      ...
      onMounted(() => {
        ...
        const coordinates = [
          [55.937071, 54.718473, "Гуманитарный корпус УУНиТ", "ул. К. Маркса 3/1", "https://uust.ru/ipch/geomuseum/"],
          [55.93341, 54.720888, "Главный корпус УУНиТ", "ул. З. Валиди 32", "https://uust.ru/ipch/zoomuseum/"],
          [55.936081, 54.720401, "Кампус Евразийского НОЦ РБ", "ул. З. Валиди 32а", "https://campus.nocrb.ru/"],
        ];
    ...
    popupPosition.value = {
      ...
      const navigateToPage = () => {
        if (popupText.value === "Гуманитарный корпус УУНиТ") {
          hidePopup(); // Плавно скрываем popup перед переходом
          setTimeout(() => {
            emit('navigateToMainK'); // Эмитируем событие для перехода на ufarob.vue
          }, 300); // Ждем, чтобы popup скрылся
        } else if (popupText.value === "Главный корпус УУНиТ") {
          hidePopup(); // Плавно скрываем popup перед переходом
          setTimeout(() => {
            emit('navigateToMainUust'); // Эмитируем событие для перехода на ufanet.vue
          }, 300); // Ждем, чтобы popup скрылся
          }
          else if (popupText.value === "Кампус Евразийского НОЦ РБ") {
          hidePopup(); // Плавно скрываем popup перед переходом
          setTimeout(() => {
            emit('navigateToNotsRb'); // Эмитируем событие для перехода на ufanet.vue
          }, 300); // Ждем, чтобы popup скрылся
          }
          };
