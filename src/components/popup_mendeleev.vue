...
<script>
  ...
  export default {
    emits: ['UUSTxumK', 'UFIX_run', 'UFIS_RUN', 'UGNTU_kat'], // Объявляем событие
    setup(_, {emit}) {
      ...
      const defaultPointStyle = new Style({
        image: new Circle({
        radius: 7,
        fill: new Fill({ color: 'rgba(0, 168, 221, 1)' }),
      ...
      onMounted(() => {
        ...
        const coordinates = [
          [55.931985, 54.719634],
          [56.038583, 54.797439, "Институт нефтехимии и катализа УФИЦ РАН", "ул. Проспект октября 141", "https://ink-ran.ru/"],
          [56.015727, 54.767287, "Главный корпус УФИХ УФИЦ РАН", "ул. Проспект октября 71", "http://ufaras.ru/?page_id=14041"],
          [56.058497, 54.818319]
        ];
    ...
    popupPosition.value = {
      ...
      const navigateToPage = () => {
        if (popupText.value === "Химический корпус УУНиТ") {
          hidePopup(); // Плавно скрываем popup перед переходом
          setTimeout(() => {
            emit('UUSTxumK'); // Эмитируем событие для перехода на ufarob.vue
          }, 300); // Ждем, чтобы popup скрылся
        } else if (popupText.value === "Главный корпус УФИХ УФИЦ РАН") {
          hidePopup(); // Плавно скрываем popup перед переходом
          setTimeout(() => {
            emit('UFIX_run'); // Эмитируем событие для перехода на ufanet.vue
          }, 300); // Ждем, чтобы popup скрылся
          }
          else if (popupText.value === "Институт нефтехимии и катализа УФИЦ РАН") {
          hidePopup(); // Плавно скрываем popup перед переходом
          setTimeout(() => {
            emit('UFIS_RUN'); // Эмитируем событие для перехода на ufanet.vue
          }, 300); // Ждем, чтобы popup скрылся
          }
          else if (popupText.value === "УГНТУ") {
          hidePopup(); // Плавно скрываем popup перед переходом
          setTimeout(() => {
            emit('UGNTU_kat'); // Эмитируем событие для перехода на ufanet.vue
          }, 300); // Ждем, чтобы popup скрылся
          }
          };
